        ...
        function atualizarSelectAtaqueArma(indice, arma) {
            ...
            const modFor = getModificadorAtributoBase('for');
            const modDes = getModificadorAtributoBase('des');
            const proficiente = arma ? formatarArma(arma).proficiente : false;
            const totalFor = modFor + (proficiente ? bonusProf : 0);
            const totalDes = modDes + (proficiente ? bonusProf : 0);

            select.innerHTML = '';

            const opcoes = [
                { value: 'for', label: `FOR (${formatarBonusCalculado(totalFor)})` },
                { value: 'des', label: `DES (${formatarBonusCalculado(totalDes)})` }
            ];
            ...
            return indicesEquipados
                .map((indiceArma, indiceSlot) => {
                    ...
                    return {
                        ...
                        propriedade: limparTextoCombate(resumirMunicaoNoTexto(obterResumoPropriedadesArma(arma, false))),
        ...
        function sincronizarEquipamentosCombate() {
            ...
            [1, 2].forEach(indice => {
                ...
                const textoPropriedade = arma ? limparTextoCombate(resumirMunicaoNoTexto(obterResumoPropriedadesArma(arma, false))) : "";
        ...
        function formatarBonusProf() {
            ...
            let valor = parseInt(input.value.replace(/[^\d-]/g, "")) || 0;
        ...
        function obterResumoBonusAtaqueArma(arma) {
            const rotuloAtributo = formatarRotuloAtributo(arma?.ataque) || "-";
            const modificadorAtaque = getModificadorBaseAtributo(arma?.ataque);
            const bonusProf = getBonusProf();
            const totalAtaque = modificadorAtaque === null ? 0 : bonusProf + modificadorAtaque;

            return {
                rotuloAtributo,
                totalAtaque,
                textoResumo: `${totalAtaque >= 0 ? "+ " : "- "}${Math.abs(totalAtaque)} (${rotuloAtributo})`
            };
        }

        function obterSubtituloModalAtaque(arma) {
            const propriedades = limparTextoCombate(obterResumoPropriedadesArma(arma, true));
            let texto = propriedades || "Sem propriedades";

            if (arma?.usaMunicao && arma.arma_municao_id) {
                ...
                if (nomeMunicao) {
                    texto += ` | Muni̤̣o: ${quantidadeMunicao || "0"} ${nomeMunicao}`;
                }
            }
        ...
        function atualizarEstadoCriticoModalAtaque() {
            ...
            if (!campoDado || !botaoSucesso) return;
        ...
        function abrirEtapaDanoAtaque() {
            ...
            subtitulo.textContent = 'Digite o dano causado.';
            resumo.textContent = `Dado: ${dado} | Ataque: ${estadoAtaqueArma.totalAtaque >= 0 ? "+" : "-"}${Math.abs(estadoAtaqueArma.totalAtaque)} (${estadoAtaqueArma.rotuloAtributo})`;
        ...
        function atualizarSubAbaArmas() {
            ...
            armas.forEach(arma => {
                ...
                const rotuloAtaque = formatarRotuloAtributo(arma.ataque) || "-";
                const modificadorAtaque = getModificadorBaseAtributo(arma.ataque);
                const bonusProf = getBonusProf();
                const totalAtaque = modificadorAtaque === null ? null : bonusProf + modificadorAtaque;
                const textoAtaque = totalAtaque === null
                    ? rotuloAtaque
                    : `${rotuloAtaque} (${totalAtaque >= 0 ? "+" : ""}${totalAtaque})`;
