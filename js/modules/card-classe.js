...
function configurarImportacaoManualCard() {
  ...
  if (!fileInput || fileInput.dataset.listenerConfigurado === "true") return;
...
function aplicarNaFicha() {
  ...
  const classeAtual = document
    ...
    .replace(" ", "");
  const subclasseAtual = document.getElementById("selectSubclasse").value || "";
...
function atualizarAtalhoConsultaCaracteristicaClasse() {
  const botao = document.getElementById("abrirConsultaCaracteristicaClasse");
  if (!botao) return;
  ...
  const temTabela =
    !!dadosClasse && !!document.querySelector("#cardClasseContainer table");
...
async function carregarJSONClasse(caminho, classe = "") {
  ...
  if (!caminho && dadosEmbutidos) {
    return dadosEmbutidos;
  }

  // Constrói uma lista de candidatos de caminho para tentar (relative/./relative)
  const candidatos = [];
  if (caminho) {
    candidatos.push(caminho);
    if (!/^(?:\.\/|\/|https?:)/i.test(caminho)) {
      candidatos.push(`./${caminho}`);
      candidatos.push(String(caminho).replace(/^\/+/, ""));
    }
  } else {
    const padrao = `caracteristica_${normalizarTextoClasse(classe)}.json`;
    candidatos.push(`./Classes/${padrao}`);
    candidatos.push(`Classes/${padrao}`);
  }

  // Tenta fetch nos candidatos sequencialmente
  for (const c of candidatos) {
    try {
      ...
      console.debug(
        `[carregarJSONClasse] falha fetch ${c}:`,
        err?.message || err,
      );
    }
  }

  // Fallback via XHR para compatibilidade com ambientes file:// (tentando mesmos candidatos)
  for (const c of candidatos) {
    try {
      const texto = await new Promise((resolve, reject) => {
        const request = new XMLHttpRequest();
        request.open("GET", c, true);
        request.onload = function () {
          const statusValido = request.status >= 200 && request.status < 300;
          const arquivoLocalValido =
            request.status === 0 && request.responseText;
          if (!statusValido && !arquivoLocalValido) {
            reject(new Error(`XHR status ${request.status}`));
            return;
          }
      ...
      try {
        ...
        console.warn(
          `[carregarJSONClasse] parse falhou em ${c}:`,
          e.message || e,
        );
      }
    } catch (err) {
      console.debug(
        `[carregarJSONClasse] falha XHR ${c}:`,
        err?.message || err,
      );
    }
  }
  ...
  throw new Error(
    `Não foi possível carregar JSON da lista de caminhos: ${JSON.stringify(candidatos)}`,
  );
}

function importarJSONNoCard(conteudoJSON, origem = "JSON", classe = "") {
  const data =
    typeof conteudoJSON === "string" ? JSON.parse(conteudoJSON) : conteudoJSON;
