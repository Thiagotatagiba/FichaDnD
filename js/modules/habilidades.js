...
document.querySelectorAll(".pericia-prof").forEach((label) => {
  const periciaLinha = label.closest(".pericia-linha");
  if (!periciaLinha) return;

  label.setAttribute("role", "button");
  label.setAttribute("tabindex", "0");
  label.setAttribute(
    "title",
    "Clique para alternar o nível de proficiência da perícia",
  );

  label.addEventListener("click", (evento) => {
    evento.preventDefault();
    if (typeof alternarEstadoProficiencia === "function") {
      alternarEstadoProficiencia(periciaLinha);
    }
    atualizarPericias();
    atualizarPassivas();
  });

  label.addEventListener("keydown", (evento) => {
...
if (typeof inicializarEstadosPericias === "function") {
  inicializarEstadosPericias();
}
...
document.getElementById("classeNivelID").addEventListener("change", () => {
  atualizarVidaMaxNivelUm();
  normalizarPontosDeVida();
});
...
document
  ...
  ?.addEventListener("input", () => sincronizarResumoCaracteristicas(true));
document
  .getElementById("subclasse")
  ?.addEventListener("change", () => sincronizarResumoCaracteristicas(true));
document
...
const overlayIniciativa = document.getElementById("modalIniciativaOverlay");
...
document
  ...
  ?.addEventListener("click", () => {
    ...
    animarRolagemNoCampo(campoDano, qtd, faces, 1000, 60).then((rolagem) => {
      botao.classList.remove("rolando");
...
document.getElementById("modalAtaqueDano")?.addEventListener("input", () => {
  atualizarTotalModalDano();
});

document.getElementById("modalAtaqueDano")?.addEventListener("change", () => {
  atualizarTotalModalDano();
});
