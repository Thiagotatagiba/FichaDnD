...
function formatarRotuloAtributo(chaveAtributo) {
  ...
  const campoClasseMagia = document.getElementById("magiaClasseConjuradora");
  if (campoClasseMagia) campoClasseMagia.setAttribute("readonly", "readonly");
...
function obterSubtituloTalento(categoria, talento) {
  const config = TALENTOS_CONFIG[categoria];
  const tipoPadrao = categoria === "outros" ? "Talento" : "Característica";
...
function converterTextoParaTalentos(texto, categoria) {
  return String(texto || "")
    ...
    .filter(Boolean)
    .map((nome) => normalizarItemTalento({ nome, descricao: "" }, categoria));
}
...
function renderTalentos() {
  ...
  Object.entries(TALENTOS_CONFIG).forEach(([categoria, config]) => {
    ...
    itens.forEach((talento, indice) => {
      ...
      botao.title =
        String(talento?.descricao || "").trim() || "Sem descrição cadastrada.";
...
function abrirModalTalento(
  ...
  estadoModalTalento.itemOriginal = normalizarItemTalento(talento, categoria);
  ...
  if (titulo)
    titulo.textContent =
      talento?.nome || (categoria === "outros" ? "Talento" : "Característica");
  if (subtitulo)
    subtitulo.textContent = obterSubtituloTalento(categoria, talento);
  if (botaoSalvar)
    botaoSalvar.style.display = modo === "editar" ? "inline-flex" : "none";
  if (botaoEditar)
    botaoEditar.style.display = modo === "visualizar" ? "inline-flex" : "none";
  if (botaoExcluir)
    botaoExcluir.style.display = modo === "visualizar" ? "inline-flex" : "none";
...
function salvarModalTalento() {
  ...
  const campoDescricao = document.getElementById("talentoModalDescricaoInput");
  ...
  if (
    ...
    estadoModalTalento.indice = talentosEstruturados[categoria].length - 1;
  }
...
function normalizarMagiaRegistro(registro, nivel) {
  ...
  const propriedades = {
    ...
    concentracao:
      !!registro?.propriedades?.concentracao || tags.includes("concentracao"),
  };
  const componentes = {
    verbal: !!registro?.componentes?.verbal || tags.includes("verbal"),
    somatico: !!registro?.componentes?.somatico || tags.includes("somatico"),
    material: !!registro?.componentes?.material || tags.includes("material"),
  };
  ...
  const efeitos = Array.isArray(registro?.efeitos)
    ? registro.efeitos
        ...
        .filter(
          (efeito) =>
            efeito.tipo || efeito.formula || efeito.dano_tipo || efeito.regra,
        )
    : [];
...
function consumirEspacoMagia(nivelSlot) {
  ...
  if (espaco.campoRest) {
    ...
    espaco.campoRest.dispatchEvent(new Event("input", { bubbles: true }));
    espaco.campoRest.dispatchEvent(new Event("change", { bubbles: true }));
    return true;
  }
...
function registrarHistoricoMagia(magia, nivelSlot = null) {
  ...
  adicionarRegistroHistoricoCombate(tipoHistorico, valorHistorico, "", "Ação", {
    timeline: {
...
function abrirHistoricoCombateMagia() {
  ...
  const botaoHistorico = [...document.querySelectorAll(".subaba-botao")].find(
    (botao) =>
...
function obterEfeitoDanoMagia(magia) {
  return (
    (magia?.efeitos || []).find((efeito) => efeito?.tipo === "dano") || null
  );
}
...
function renderizarCardDetalheMagia(
  ...
  descricao.textContent = [
    magia.descricao || "",
    magia.proximos_niveis ? `Próximos níveis: ${magia.proximos_niveis}` : "",
  ]
    .filter(Boolean)
    .join("\n\n");
...
function renderizarOpcoesSlotMagia(magia) {
  ...
  const botaoConfirmar = document.getElementById("magiaSlotModalConfirmar");
  if (!container) return;
...
function abrirModalUsarMagia(magia, opcoes = {}) {
  ...
  estadoModalUsoMagia.nivelMinimo = Math.max(1, parseInt(magia.nivel, 10) || 1);
...
function obterResumoStatusMagico() {
  return {
    ...
    cd: String(document.getElementById("magiaCD")?.value || "-").trim() || "-",
    ataque:
      String(
        document.getElementById("magiaAtaqueMagico")?.value || "-",
      ).trim() || "-",
...
function alternarGrupoMagia(grupo) {
  ...
  if (header) header.setAttribute("aria-expanded", fechado ? "false" : "true");
}
...
function atualizarSubAbaMagiasCombate() {
  ...
  NIVEIS_MAGIA.forEach((nivelGrupo) => {
    ...
    const espaco = nivelGrupo > 0 ? obterEstadoEspacoMagia(nivelGrupo) : null;
...
function obterMagiaDoEstadoModal() {
  ...
  return (
    magias[estadoModalMagia.indice] || criarMagiaPadrao(estadoModalMagia.nivel)
  );
}
  ...
  return normalizarMagiaRegistro(
    {
      ...
      componentes,
      resolucao,
      teste_resistencia: document.getElementById("magiaModalTesteResistencia")
        .value,
      efeitos,
...
function atualizarVisibilidadeFormularioMagia() {
  ...
  const habilitaCura = document.getElementById("magiaModalPropCura")?.checked;
  const resolucao = document.getElementById("magiaModalResolucao")?.value;
  const danoComResolucao = !!habilitaDano && !!String(resolucao || "").trim();
  ...
  document
    ...
    ?.classList.toggle("oculto", !(habilitaDano && resolucao === "Ataque"));
  document
...
function abrirModalMagia(nivel, indice, modo = "visualizar", opcoes = {}) {
...
function excluirMagiaAtual() {
  const magias = lerMagiasNivel(estadoModalMagia.nivel);
  if (estadoModalMagia.indice === null || !magias[estadoModalMagia.indice])
    return;
...
function sincronizarResumoCaracteristicas(forcar = false) {
  ...
  const containerSubclasse = document.getElementById("containerSubclasse");

  if (campoSubclasse && campoSubclasseResumo && secaoSubclasse) {
    const subclasseDisponivel =
      !!containerSubclasse && !containerSubclasse.classList.contains("hidden");
...
function getMultiplicadorProficiencia(estado) {
...
function calcularBonusPorEstado(bonusProf, estado) {
  const nivel = Number(estado);
  if (nivel === 1) {
    return bonusProf;
  }
  if (nivel === 2) {
    return Math.floor(bonusProf / 2);
  }
  if (nivel === 3) {
    return bonusProf * 2;
  }
...
function atualizarIndicadorEstadoProficiencia(periciaLinha) {
  const estado = Number(periciaLinha.dataset.proficiencia) || 0;
  const indicador = periciaLinha.querySelector(".pericia-prof-indicador");
  const labels = [
    "Sem proficiência",
    "Proficiente",
    "Meia proficiência",
  ...
  const icones = ["◯", "⬤", "◐", "★"];
  if (indicador) {
    indicador.textContent = icones[estado] || "◯";
  }
  const label = periciaLinha.querySelector(".pericia-prof");
  if (label) {
    const periciaNome = periciaLinha
      .querySelector(".pericia-nome")
      ?.textContent?.trim();
    label.setAttribute(
      "aria-label",
      `${labels[estado] || labels[0]} em ${periciaNome}`,
    );
  }
}

function alternarEstadoProficiencia(periciaLinha) {
  const atual = Number(periciaLinha.dataset.proficiencia) || 0;
  const proximo = (atual + 1) % 4;
  periciaLinha.dataset.proficiencia = String(proximo);
  atualizarIndicadorEstadoProficiencia(periciaLinha);
}

function inicializarEstadosPericias() {
  document.querySelectorAll(".pericia-linha").forEach((periciaLinha) => {
    if (!periciaLinha.dataset.proficiencia) {
      periciaLinha.dataset.proficiencia = "0";
    }
    const label = periciaLinha.querySelector(".pericia-prof");
    if (!label) return;
    if (!label.querySelector(".pericia-prof-indicador")) {
      const span = document.createElement("span");
      span.className = "pericia-prof-indicador";
...
function atualizarPericias() {
  ...
  document.querySelectorAll(".pericia-linha").forEach((pericia) => {
    ...
    const estado = Number(pericia.dataset.proficiencia) || 0;
    const bonus = calcularBonusPorEstado(bonusProf, estado);
    const total = modificadorBase + bonus;
...
function atualizarPassivas() {
  const bonusProf = getBonusProf();
  const sabValor = parseInt(document.getElementById("atrib_sab")?.value) || 0;
  const sabMod = Math.floor((sabValor - 10) / 2);
  const sabProf = document.getElementById("prof_salv_sab")?.checked
    ? bonusProf
    : 0;
  const percepcao =
    parseInt(document.getElementById("periciaPercepcaoValor")?.value) || 0;
  const intuicao =
